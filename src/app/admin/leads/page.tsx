...
import { listLandingLeads } from "@/lib/leads/list-leads";
...
export default async function AdminLeadsPage() {
  const { rows, error } = await listLandingLeads(150);

  return (
    <div className="min-h-dvh bg-[var(--background)] text-[var(--color-text)]">
      <div className="mx-auto max-w-5xl bg-white px-5 py-10 md:px-10 md:py-14">
        <header className="border-b border-rule pb-6">
          <p className="type-caption uppercase tracking-[0.12em] text-body/70">Internal</p>
          <h1 className="mt-2 type-h2 text-brand">Landing leads</h1>
          <p className="mt-3 max-w-2xl text-sm leading-relaxed text-body">
            Fallback log written before notification email. Partial and full submissions both
            appear here so a mailbox problem cannot hide a lead.
          </p>
        </header>
